...
export default {
  ...
  methods: {
    ...
    async loadVectorData(geoJSON, name) {
      try {
        ...
        if (vectorItem.isDynamic){
          ...
          //设置相机变化监听
          this.setupCamerListener(vectorItem);

          //初始加载
          await this.loadPointsPage(vectorItem);
          
          
        } else{
          //静态数据处理
          this.processStaticData(vectorItem,geoJSON)
        }
      ...
      this.showErrorMessage(`加载 ${name} 数据失败: ${e.message}`);
    }},

    //设置相机监听
    setupCamerListener(vectorItem){
      if(vectorItem.handler){
        vectorItem.handler.destroy();
      }
      
      vectorItem.handler = new Cesium.ScreenSpaceEventHandler(this.viewer.scene.canvas);

      //相机移动开始
      this.viewer.camera.moveStart.addEventListener(()=>{
        vectorItem.isPaused = false;
      });

      //相机移动结束
      this.viewer.camera.moveEnd.addEventListener(()=>{
        vectorItem.isPaused = false;
        // vectorItem.currentPage = 0;
        // this.loadPointsPage(vectorItem)
      })
    },
    //分页加载点数据
    async loadPointsPage(vectorItem){
      console.log(vectorItem.isLoading);
      console.log(vectorItem.isPaused);
      if(vectorItem.isLoading || vectorItem.isPaused)return;
      try{
        vectorItem.isLoading = true;
        
        //获取当前识图范围
        vectorItem.bounds = this.viewer.camera.computeViewRectangle();
        const { west, south, east, north} = vectorItem.bounds;

        //显示加载状态
        this.showLoadingIndicator(true, `正在加载 ${vectorItem.name} (第${vectorItem.currentPage + 1}页)...`)
        //请求数据
        const response = await fetch(
          `http://localhost:5000/vector-tiles?` +
      `minLon=${Cesium.Math.toDegrees(west)}&minLat=${Cesium.Math.toDegrees(south)}` +
      `&maxLon=${Cesium.Math.toDegrees(east)}&maxLat=${Cesium.Math.toDegrees(north)}` +
      `&zoom=10&layer=${vectorItem.name}` +
      `&page=${vectorItem.currentPage}&pageSize=${vectorItem.pageSize}`
        );
        ...
        if(pointFeatures.length >0){
          ...
          await this.addPoints(vectorItem.points, pointFeatures);

          //更新页码
          vectorItem.currentPage = vectorItem.currentPage + 1;

          //如果还有数据，继续加载
          console.log(pointFeatures.length)
          console.log(vectorItem.pageSize)
          if(pointFeatures.length === vectorItem.pageSize){
            console.log(vectorItem.currentPage); 
            requestIdleCallback(() => this.loadPointsPage(vectorItem));
          } else{
            this.showLoadingIndicator(false);
          }
        }
        ...
        //首次加载时飞向数据
        if (vectorItem.currentPage === 1){
          this.flyToFeatures(vectorItem.pointFeatures);
        }
      }catch (error){
        console.error('加载分页数据失败');
        console.log(error.message);
        this.showLoadingIndicator(false);
        this.showErrorMessage(`加载 ${vectorItem.name} 失败: ${error.message}`);
      }finally{
        vectorItem.isLoading = false;
        // this.setupPointClickHandler(vectorItem);
      }
    },
    
    processStaticData(vectorItem, geoJSON){
      //验证数据
      ...
      if (pointFeatures.length > 0){
        ...
        this.setupPointClickHandler(vectorItem);
        this.flyToFeatures(pointFeatures);
      }

      //处理其他几何类型
      if (otherFeatures.length > 0){
        this.loadOtherFeatures(vectorItem, otherFeatures);
      }

    },

    async loadOtherFeatures(vectorItem, features){
      ...
      this.viewer.dataSource.add(vectorItem.dataSource);
      this.setupPointClickHandler(vectorItem);

      //飞向数据范围
      await this.viewer.flyToFeatures(vectorItem.dataSource);

    },

    //新增：设置点要素点击事件
    setupPointClickHandler(vectorItem){
      ...
      handler.setInputAction((movement) => {
        ...
        if(picked.primitive){
          const position = picked.primitive.position;
          const carto = Cesium.Cartographic.fromCartesian(position);
          const lon = Cesium.Math.toDegrees(carto.longitude);
          const lat = Cesium.Math.toDegrees(carto.latitude);

          const feature = vectorItem.pointFeatures.find(f =>{
            const [fLon, fLat] = f.geometry.coordinates;
            return Math.abs(fLon - lon) < 0.0001 && Math.abs(fLat - lat) < 0.0001;
          });

          if(feature.properties){
            this.showFeatureAttributes(feature.properties);
          }
        }
      }, Cesium.ScreenSpaceEventType.LEFT_CLICK);

      vectorItem.handler = handler;
    },

    //新增显示要素属性
    showFeatureAttributes(properties){
      this.currentAttributes = properties;
      this.attributeColumns = Object.keys(properties).map(key => ({
        prop:key,
        label:key
      }));
    ...
    showLoadingIndicator(show, message=''){
      if (!this.loadingIndicator){
        this.loadingIndicator = document.createElement('div');
        //样式设计
        this.viewer.container.appendChild(this.loadingIndicator);
      }

      this.loadingIndicator.style.display = show ? 'block' : 'none';
      if(message) this.loadingIndicator.textContent = message;
